...
export const followStories = (user_id, story) => dispatch => {
  APIUtil.followStory(user_id, story)
    .done(story => dispatch(receiveStory(story)))
    .fail(errors => dispatch(receiveErrors(errors)))
};

export const findFollows = id => dispatch => (
  APIUtil.findFollows (id)
    .done(follow => dispatch(receiveFollow(follow)))
    .fail(errors => dispatch(receiveErrors(errors)))
);