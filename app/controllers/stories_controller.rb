...
class StoriesController < ApplicationController
  ...
  def follow_story
    @follow = current_user.follows.where(story_id: params[:story_id])
    if @follow.present?
      @follow = Follow.find_by_story_id(params[:story_id])
      @follow.delete
    else
      @follow = Follow.new(user_id: params[:user_id], story_id: params[:story_id])
      @follow.save
    end
    render :follow
  end
  ...
  def find_follow
    @follows = Follow.where(story_id: params[:id])
    render :find_follow
  end

  def update
    update_params, = story_params
    @story = find_story(params[:id])
  ...
  def story_params
    result = params.require(:story).permit(
      ...
      :kind, :state, :points,
      :title, :description, :follow, :priority,
      tasks: {}
    )
    result[:title]&.strip!
